...
from site_mirror import mirror_site
...
class CLISecurityServer:
    ...
    async def run_mirror(
        ...
        """Mirror a website into /app/artifacts/<session_id>/mirror/.

        Runs the two-phase download (wget + URL extraction) and returns
        a JSON summary with file counts, sizes, and types.
        """
        import os
        output_dir = os.path.join(self.working_dir, session_id, "mirror")
        os.makedirs(output_dir, exist_ok=True)

        logger.info("Mirroring %s → %s (depth=%d, timeout=%d)",
                     url, output_dir, depth, global_timeout)
        ...
        result = await loop.run_in_executor(
            None,
            lambda: mirror_site(
                url, output_dir, depth=depth, global_timeout=global_timeout,
            ),
        )
        return result
    ...
    async def run_secret_scan(
        ...
        """Scan a mirrored site directory for secrets and sensitive data.

        If *directory* is not given, defaults to the mirror output:
        ``/app/artifacts/<session_id>/mirror/``.
        """
        import os
        if directory is None:
            directory = os.path.join(self.working_dir, session_id, "mirror")

        if not os.path.isdir(directory):
            return {"error": f"Directory not found: {directory}"}

        logger.info("Scanning secrets in %s (max_findings=%d)",
                     directory, max_findings)

        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(
            None,
            lambda: scan_directory(directory, max_findings=max_findings),
        )
        return result
